...
WAMのCSVファイルを基本情報として扱うことにしたので以下は古い情報。  
都道府県ごとのGoogleスプレッドシートなどは全て廃止。
...
2022年1月からWAMでCSVファイルが公開開始。  
https://www.wam.go.jp/content/wamnet/pcpub/top/sfkopendata/

### Googleスプレッドシートへのデータ入力の注意点・共通版
- 英数字は半角で入力。
- 「事業所番号」は必須。グループホームごとに重複しない番号を指定。元から重複してる場合はそのまま残す（最後に読み込んだデータで上書きされる）。
...
- Googleスプレッドシートのデータ以外はサイト側でグループホーム事業者がユーザー登録して管理事業者として申請すれば直接変更可能。
